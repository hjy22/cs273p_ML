...
## METHODS ################################################################
    ...
    def plotBoundary(self,X,Y):
        ...
        x2b = -(self.theta[0]+x1b*self.theta[1])/self.theta[2];   
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                if YY[i]:
                    ...
                    gradi = si*XX[i,:]             # TODO: compute gradient of NLL loss
            ...
            Jnll.append( Jsur ) # TODO evaluate the current NLL los
            plt.pause(.01);  
    ...
    def regTrainL1(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000,alpha=0.,plot=None):
        """ Train the logistic regression using stochastic gradient descent """
        M,N = X.shape;                     # initialize the model if necessary:
        self.classes = np.unique(Y);       # Y may have two classes, any values
        XX = np.hstack((np.ones((M,1)),X)) # XX is X, but with an extra column of ones
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                if YY[i]:
                    ...
                    gradi = si*XX[i,:]             # TODO: compute gradient of NLL loss
                gradi += alpha*np.sign(self.theta+1e-200)
                self.theta -= stepsize * gradi;  # take a gradient step
            ...
            ##  Jsur = sum_i [ (log si) if yi==1 else (log(1-si)) ]
            S = 1./(1.+np.exp(-(XX.dot(self.theta))))
            Jsur = -np.mean(YY*np.log(S)+(1-YY)*np.log(1-S))+ alpha*np.sum(np.abs(self.theta))
            Jnll.append( Jsur ) # TODO evaluate the current NLL los
            plt.pause(.01);  
            done = epoch>=stopEpochs or (epoch>1 and abs(Jnll[-1]-Jnll[-2])<stopTol)
        plt.figure(1); plt.plot(Jnll,'b-',J01,'r-'); plt.draw();    # plot losses
    ...
    def regTrainL2(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000,alpha=0.,plot=None):
        """ Train the logistic regression using stochastic gradient descent """
        M,N = X.shape;                     # initialize the model if necessary:
        self.classes = np.unique(Y);       # Y may have two classes, any values
        XX = np.hstack((np.ones((M,1)),X)) # XX is X, but with an extra column of ones
        ...
        while not done:
            ...
            ##  Jsur = sum_i [ (log si) if yi==1 else (log(1-si)) ]
            S = 1./(1.+np.exp(-(XX.dot(self.theta))))
            Jsur = -np.mean(YY*np.log(S)+(1-YY)*np.log(1-S))+ alpha*np.sum(np.square(self.theta))
            Jnll.append( Jsur ) # TODO evaluate the current NLL los
            plt.pause(.01);  
            done = epoch>=stopEpochs or (epoch>1 and abs(Jnll[-1]-Jnll[-2])<stopTol)
        plt.figure(1); plt.plot(Jnll,'b-',J01,'r-'); plt.draw();    # plot losses
